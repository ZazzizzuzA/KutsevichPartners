...
export default {
  ...
  data() {
    return {
      language,
      posts,
      InteriorsPosts: this.InteriorsPosts
    };
  },
  methods: {
    even: function(arr) {
  ...
  created: function() {
    this.InteriorsPosts = this.posts[this.language.numOfLang].filter(el => el.type === 'interior')
      .sort((a, b) => {
        if (a.typeId < b.typeId) return -1;
        if (a.typeId > b.typeId) return 1;
        return 0
      });
    this.InteriorsPosts.forEach( post => {
